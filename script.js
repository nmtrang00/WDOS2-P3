...
function generateField(field, formType, onchange){
    ...
    if (["radio","checkbox"].includes(formType))
    {
        ...
        {
            let input_text = `<input type="${formType}" id="${opts[i]}" name="type" value="${opts[i]}" onchange="${onchange}">` ;
...
function checkOut(){
    ...
    document.getElementById("confirmation").innerHTML="Thank you. We have well received your order.";
    document.querySelector( 'input[name="type"]:checked').checked=false;
    for (let field of ["coffee", "size"]){
        clearSelectOptions(field);
        document.getElementById(field).innerHTML+='<option value="Default">Please choose the type of coffee first</option>';
    }
    let markedCheckbox = document.querySelectorAll('input[type="checkbox"]');  
    for (let i = 0; i<markedCheckbox.length; i++) {  
        if (markedCheckbox[i].checked){
    ...
    document.getElementById("favDrink").innerHTML='<br><b>FAVORITE DRINK:</b><br>';
    document.getElementById("cart").innerHTML='<p><b>CART:</b></p>';
...
console.log(fetch('coffeeShop.json'));